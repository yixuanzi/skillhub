...
class TestTokenPlaceholderReplacement:
    """测试 token 占位符替换功能."""

    def test_simple_string_replacement(self, db: Session):
        """测试简单的字符串替换."""
        import uuid
        ...
        mtoken_obj = MTokenService.create(db, mtoken, user.id)
    ...
    def test_multiple_placeholders_in_one_string(self, db: Session):
        """测试一个字符串中的多个占位符."""
        # 创建测试用户
        user = User(
            username="testuser2",
    ...
    def test_nested_dict_replacement(self, db: Session):
        """测试嵌套字典的替换."""
        # 创建测试用户
        user = User(
            username="testuser3",
    ...
    def test_list_replacement(self, db: Session):
        """测试列表中的替换."""
        # 创建测试用户
        user = User(
            username="testuser4",
        ...
        # 验证列表中的占位符被替换
        assert result["headers"][0] == "value1"
        assert result["headers"][1] == "value2"

        print("✅ 列表替换测试通过")

    def test_placeholder_not_found(self, db: Session):
        """测试占位符找不到时的行为."""
        # 创建测试用户
        user = User(
            username="testuser5",
    ...
    def test_empty_and_none_values(self, db: Session):
        """测试空值和 None 的处理."""
        # 创建测试用户
        user = User(
            username="testuser7",
...
if __name__ == "__main__":
    ...
    try:
        test = TestTokenPlaceholderReplacement()

        # 运行所有测试
        print("开始测试...\n")
        test.test_simple_string_replacement(db)
        test.test_multiple_placeholders_in_one_string(db)
        test.test_nested_dict_replacement(db)
        test.test_list_replacement(db)
        test.test_placeholder_not_found(db)
        test.test_complex_nested_structure(db)
        test.test_empty_and_none_values(db)
